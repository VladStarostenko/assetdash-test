...
describe('RankRepository', () => {
  ...
  it('first rank of the day becomes both the open and current rank', async () => {
    const openDate = parseAsEstDate('2020-05-07 09:31');
    const day = parseAsEstDate('2020-05-07 16:00');

    await ranksRepository.updateRank({assetId: 1, position: 100, date: openDate});

    expect(await ranksRepository.findOpenFor(day, 1))
      .to.deep.include({assetId: 1, position: 100, date: openDate});
    expect(await ranksRepository.findMostRecentFor(day, 1))
      .to.deep.include({assetId: 1, position: 100, date: openDate});
  });

  it('after adding 2 ranks for a day, first stays the open, second becomes current', async () => {
    const openDate = parseAsEstDate('2020-05-07 09:31');
    const middayDate = parseAsEstDate('2020-05-07 12:31');
    const day = parseAsEstDate('2020-05-07 16:00');
    ...
    expect(await ranksRepository.findOpenFor(day, 1))
      .to.deep.include({assetId: 1, position: 100, date: openDate});
    expect(await ranksRepository.findMostRecentFor(day, 1))
      .to.deep.include({assetId: 1, position: 101, date: middayDate});
  });

  it('3rd update updates only the current rank', async () => {
    const openDate = parseAsEstDate('2020-05-07 09:31');
    const middayDate = parseAsEstDate('2020-05-07 12:31');
    const laterDate = parseAsEstDate('2020-05-07 15:31');
    const day = parseAsEstDate('2020-05-07 16:00');
    ...
    expect(await ranksRepository.findOpenFor(day, 1))
      .to.deep.include({assetId: 1, position: 100, date: openDate});
    expect(await ranksRepository.findMostRecentFor(day, 1))
      .to.deep.include({assetId: 1, position: 102, date: laterDate});
  });
  ...
  it('filters by assetId', async () => {
    ...
    expect(await ranksRepository.findOpenFor(day, 1))
      .to.deep.include({assetId: 1, position: 100, date: openDate});
    expect(await ranksRepository.findOpenFor(day, 2))
      .to.deep.include({assetId: 2, position: 101, date: openDate});
    expect(await ranksRepository.findMostRecentFor(day, 1))
      .to.deep.include({assetId: 1, position: 100, date: openDate});
    expect(await ranksRepository.findMostRecentFor(day, 2))
      .to.deep.include({assetId: 2, position: 101, date: openDate});
  });
});
